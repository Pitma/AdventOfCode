...
class AoCSolver:
    def __init__(self, level: int = 1):
        self.level = level
        self.sample_answers = {
            1: 1227775554,  # Replace with actual sample answer for part 1
            2: 4174379265   # Replace with actual sample answer for part 2
        }
    ...
    def solve_part1(self, data: Any) -> Union[int, str]:
        ...
        invalid_ids = 0
        for item in data:
            for check in range(item[0], item[1] + 1):
                if len(str(check)) % 2 == 0:
                    mid = len(str(check)) // 2 + len(str(check)) % 2
                    if str(check)[:mid] == str(check)[mid:]:
                            invalid_ids += check
                            #print(check)
        return invalid_ids
    ...
    def solve_part2(self, data: Any) -> Union[int, str]:
        ...
        for item in data:
            for check in range(item[0], item[1] + 1):
                    if str(check) in (str(check) + str(check))[1:-1]:
                        invalid_ids += check
                        #print(check)
